...
Hence in the case $Q$ is of type  $\{ \name{seq} , \name{key} \}$ and $V$ is of type   $\{ \name{seq} , \name{val} \}$ then the output of $\text{Att}(Q,K,V)$ will have type $\{ \name{val} ,\name{seq} \}$. 
The output $Y$ satisfies the postconditions  $|\name{seq}(Y)|=|\name{seq}(Q)|$ and $|\name{val}(Y)|=|\name{val}(V)|$.
