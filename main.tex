...
Here, we propose mathematical notation for tensors with \emph{named axes}.\footnote{%
We follow NumPy in using the term \emph{axis}. Other possible terms would be \emph{index}, \emph{dimension}, or \emph{mode}, but these are all ambiguous in their own way, and we thought that \emph{axis} would be the least problematic.} The notation has a formal underpinning, but is hopefully intuitive enough that machine learning researchers can understand it without much effort. 
%
...
The $\key$ axis is for the features of queries and keys, and $\val$ is for the features of values. The $\seq$ axis is for tokens of sequences. This notation makes it clear what are the types of each input, and how they are acted upon. The tensor $Q$ is a query, which is a vector over the $\key$ axis. The tensor $K$ is a sequence of keys, and so has the $\seq$ and $\key$ axes. You can think of it as a matrix, but don't need to remember whether $\seq$ corresponds to columns and $\key$ to rows, or vice versa. The tensor $V$ is a sequence of values, and so has the $\seq$ and $\val$ axes. The description of the functions makes it clear which axis any operation acts upon. For example, to parse the expression $Q \ndot{\key} K$, it doesn't matter if $\key$ corresponds to rows or columns in $K$, since the dot product is taken over the $\key$ axis shared between $K$ and $Q$. 

Our notation also makes it easy to  ``broadcast'' or ``vectorize'' a function by extending it to act on tensors with more axes. For example, if instead of being a vector with the single axis $\key$, $Q$ is a tensor with the three axes $\key$, $\seq$ and $\batch$  (corresponding to tokens of a sequence and examples in a minibatch, respectively) then the $\text{Attention}$ function works as written, acting on each example in a minibatch in parallel.
...
Similarly, we can also add a $\heads$ axis to the inputs for multiple attention heads.

Our notation is inspired by libraries for programming with multidimensional arrays \citep{numpy,pytorch} and extensions for named axes, like xarray \citep{xarray}, Nexus \citep{chen2017typesafe}, tsalib \citep{tsalib}, NamedTensor \citep{namedtensor}, named tensors in PyTorch \citep{named-tensors}, and Dex \citep{maclaurin+:2019}. However, our focus is on mathematical notation rather than code.
...
Note that this definition computes only a single value. If we want a sequence, we can just give $Q$ a $\seq'$ axis (or some other name), and the function will compute an output sequence. Furthermore, if we give $Q$, $K$, and $V$ a $\heads$ axis for multiple attention heads, then the function will compute multi-head attention.
