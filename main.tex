...
\begin{align*}
%A \ndot{\height} B &= \sum_i A_{\height(i)} B_{\height(i)} = \nmatrix{}{\width}{
...
where
\begin{align*}
  W^l &\in \mathbb{R}^{\layer'[n_{l]} \times \layer[n_{l-1]}} \\
...
\begin{align*}
  \text{Conv2d} \colon \reals^{\chans \times \height[h] \times \width[w]} %\times \reals^{\chans \times \kh \times \kw} \times \reals
...
where
\begin{align*}
U &\in \reals^{\height[h / kh] \times \width[w / kw] \times \kh[kh] \times \kw[kw]} \\
U_{\height(i), \width(j), \kh(di), \kw(dj)} & = X_{\height(i \times kh + di -1), \width(j \times kw + dj -1)}.
\end{align*}
...
where
\begin{align*}
W^l & \in \reals^{\chans'[c_{l]} \times \chans[c_{l-1]} \times \kh[kh_l] \times \kw[kw_l]} \\
b^l &\in \reals^{\chans'[c_{l]}}
\end{align*}
and
\begin{align*}
...
Named axes are very helpful for working with discrete random variables, because each random variable can be represented by an axis with the same name. For instance, if $\avar$ and $\bvar$ are random variables, we can treat $p(\bvar \mid \avar)$ and $p(\avar)$ as tensors:
\begin{align*}
p(\bvar \mid \avar) &\in [0, 1]^{\avar \times \bvar} & \nsum{\bvar} p(\bvar \mid \avar) &= 1 \\
p(\avar) &\in [0, 1]^{\avar} & \nsum{\avar} p(\avar) &= 1
\end{align*}
Then many common operations on probability distributions can be expressed in terms of tensor operations:
\begin{align*}
p(\avar, \bvar) &= p(\bvar \mid \avar) \odot p(\avar) && \text{chain rule}\\
p(\bvar) &= \nsum{\avar} p(\avar, \bvar) = p(\bvar \mid \avar) \ndot{\avar} p(\avar) && \text{marginalization} \\
p(\avar \mid \bvar) &= \frac{p(\avar, \bvar)}{p(\bvar)} = \frac{p(\bvar \mid \avar) \odot p(\avar)}{p(\bvar \mid \avar) \ndot{\avar} p(\avar)}. && \text{Bayes' rule}
\end{align*}
...
\begin{align*}
  X &\in \reals^{\batch \times \spc} \\
C &\in \reals^{\clusters \times \spc}
\end{align*}
...
where
\begin{align*}
\nfun{\ax,\kk}{maxk} \colon \reals^{\ax} &\rightarrow \reals^{\kk} \\
\nfun{\ax,\kk}{argmaxk} \colon \reals^{\ax} &\rightarrow \{0,1\}^{\ax,\kk}
\end{align*}
are defined such that $[\nfun{\ax,k}{maxk} A]_{\kk(i)}$ is the $i$-th largest value along axis $\ax$ and $A \ndot{\ax} (\nfun{\ax,k}{argmaxk}{A}) = \nfun{\ax,k}{max} A$.
