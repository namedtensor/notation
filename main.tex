...
\begin{align*}
  \nfun{layer}{Affine} &\colon \reals^{\name{layer}} \rightarrow \reals \\
  \nfun{layer}{Affine}(x; W, b) &= W \ndot{layer} x + b
\end{align*}
where
\begin{align*}
  W &\in \reals^{\name{layer}} \\
...
X^6 &= \text{ReLU}(W^6 \ndot{layer} X^5 + b^6) & W^6 &\in \mathbb{R}^{\name{hidden}, \name{layer}} & b^6 &\in \mathbb{R}^{\name{hidden}} \\
O &= \nfun{classes}{softmax} (W^7 \ndot{hidden} X^6 + b^7) & W^7 &\in \mathbb{R}^{\name{classes}, \name{hidden}} & b^7 &\in \mathbb{R}^{\name{classes}}
\end{align*}
