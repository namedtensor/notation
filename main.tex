...
\begin{align*}
  ...
  b^l &\in \mathbb{R}^{\nset{layer'}{n_l}}.
\end{align*}
Now $\text{FullConn}^l$ encapsulates both the equation for layer $l$ as well as its parameters (analogous to what TensorFlow and PyTorch call \emph{modules}). Unlike before, we're now using the same axis name \name{layer} for all the layers. So $\text{FullConn}^l$ temporarily computes its output in axis \name{layer'}, then renames it back to \name{layer}.
