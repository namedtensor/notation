...
U &\in \reals^{\name{channels} \times \nset{height}{h-kh+1} \times
\nset{width}{w-kw+1} \times \name{kh} \times \name{kw}}  \\
U_{\nidx{height}{i}, \nidx{width}{j}, \nidx{kh}{ki}, \nidx{kw}{kj}} &= X_{\nidx{height}{i+ki-1}, \nidx{width}{j+kj-1}}  
\end{align*}
...
We can define a single generic normalization layer, parameterized by a named index set \name{d}:
\begin{align*}
...
\begin{align*}
Y &= \nfun{batch}{xnorm}(X) && \text{batch normalization} \\
Y &= \nfun{layer}{xnorm}(X) && \text{instance normalization} \\
Y &= \nfun{layer,channels}{xnorm}(X) && \text{layer normalization}
\end{align*}
...
We define a Transformer used autoregressively as a language model. The input is a sequence of one-hot vectors $I$, from which we compute word embeddings and positional encodings. Let $\dmodel$ be a positive integer. 
\begin{align*}
  \name{layer} &\ndef [\dmodel] \\
  I &\in \{0, 1\}^{{\nset{seq}{n} \times \name{vocab}}} & \nsum{vocab} I &= 1\\
  E &\in \reals^{\name{vocab} \times \name{layer}} \\
  W &= (E \ndot{vocab} I)\sqrt{\dmodel} \\
  P &\in \reals^{\nset{seq}{n} \times \name{layer}} \\
...
\begin{align*}
  ...
  \text{SelfAtt}_n(X; W^Q, W^K, W^V, W^O) &= \nsum{heads} W^O \ndot{val} \nmov{seq'}{seq}{\text{Att}(Q, K, V, M)}
\end{align*}
where
\begin{align*}
  Q &= \nmov{seq}{seq'}{W^Q \ndot{layer} X} \\
