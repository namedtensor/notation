...
\end{align*}
The summation, $\max$, and $\min$ operators are overloaded. When applied to a tensor, they are reductions; when applied to a set of tensors, they are elementwise operations.

You can also write multiple names to perform the reduction over multiple axes at once. Moreover, if a reduction applied to a tensor has no axis name under it, the reduction is performed over all axes (as in NumPy and its derivatives).
...
If a contraction $A \ndot{} B$ has no axis name under it, the reduction is performed over all axes that $A$ and $B$ have in common.
...
Thanks to Ekin Aky\"{u}rek, Colin McDonald, Adam Poliak, Matt Post, Chung-chieh Shan, and Nishant Sinha for their input to this document (or the ideas in it).
