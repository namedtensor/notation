...
\paragraph{Advanced indexing} A common operation in array-based programming is \emph{advanced indexing}, whose semantics is vague\footnote{~\url{https://numpy.org/doc/stable/reference/arrays.indexing.html}.} and cannot be easily reasoned over. Given a tensor $A \in X^{\name{i}_1:n_1, \cdots, \name{i}_D:n_D}$, in NumPy (or PyTorch), $A$ can be indexed in the form of $A[I^1, \cdots, I^D]$, where each \emph{indexer} $I^{d \in [D]}$ can be of the following three types:
\begin{itemize}
...
\begin{align*}
  \mathrm{advIdx}&: X^{\mathcal{S}} \times ([n_1]^\mathcal{T} \times \cdots \times [n_D]^\mathcal{T}) \to X^{\mathcal{T}} \\
  B = A[I^1, \cdots, I^d] &= \mathrm{advIdx}(A, (I^1, \cdots, I^d)) \in X^{\mathcal{T}} \\
  B_{\name{j}_1:y_1, \cdots, \name{j}_E:y_E}  &= A_{\name{i}_1: I^1_{\name{j}_1:y_1, \cdots, \name{j}_E:y_E}, \cdots, \name{i}_D: I^D_{\name{j}_1:y_1, \cdots, \name{j}_E:y_E}} \\
\end{align*}
...
\begin{align*}
  \mathrm{advIdx}'&: X^{\mathcal{S}} \times ([n_1]^{\mathcal{T}_1} \times \cdots \times [n_D]^{\mathcal{T}_D}) \to X^{\bigsqcup_{i=1}^D \mathcal{T}_i} \\
  B = A[I^1, \cdots, I^d] &= \mathrm{advIdx}(A, \mathrm{broadcast}(I^1, \cdots, I^d)) \in X^{\mathcal{T}} \\
\end{align*}

Let's see a concrete example in natural language processing. Given a batch of encoded sentences using a contextualizer like BERT, we get a tensor $X \in \mathbb{R}^{\name{batch}:B, \name{sentLen}: N, \name{emb}: E}$. For each sentence, we would like to take out the encodings of a specific span $[l_b, r_b)$ for each sentence $b \in [B]$ in the batch, resulting in a tensor $ Y \in \mathbb{R}^{\name{batch}:B, \name{spanLen}: M, \name{emb}: E}$.
