...
Let $[n] = \{1, \ldots, n\}$. We deal with shapes of the form $\nset{i\sub1}{[n_1]} \times \cdots \times \nset{i\sub{r}}{[n_r]}$ so frequently that we define the shorthand $\nset{i\sub1}{n_1} \times \cdots \times \nset{i\sub{r}}{n_r}$.

Let $F$ be a field and let $\mathcal{T}$ be a shape. Then a \emph{named tensor over $F$ with shape $\mathcal{T}$} is a mapping from $\tupleshape \mathcal{T}$ to $F$. We write the set of all named tensors with shape $\mathcal{T}$ as $F^{\mathcal{T}}$.
