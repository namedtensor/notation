...
\begin{align*}
  ...
  T \nndot{foo'}{foo} A &= \sum_i T_{\nidx{foo'}{i}} A_{\nidx{foo}{i}} = \nmatrix{foo}{bar}{
    30 & 10 & 40 \\
    100 & 500 & 900
  }.
\end{align*}
The prime on $\name{foo'}$ acts kind of like a transpose, but you can use whatever naming convention you want (we also considered $\name{foo^\ast}$).
...
\text{conv1d}(X; W, b) &= W \ndot{channels' \mid channels, kernel} U + b
\end{align*}
where
\begin{align*}
W &\in \reals^{\name{channels'} \times \name{kernel}} \\
...
\begin{align*}
  ...
  &\rightarrow \reals^{\nset{height}{h-|\name{kh}|+1} \times \nset{width}{w-|\name{kw}|+1}} \\
  \text{conv2d}(X; W, b) &= W \ndot{channels' \mid channels, kh, kw} U + b
\end{align*}  
where
\begin{align*}
W &\in \reals^{\name{channels'} \times \name{kh} \times \name{kw}} \\
...
\begin{align*}
  ...
  K &= W^{l,K} \ndot{layer} \nmov{seq}{seq'}{X} & W^{l,K} &\in \mathbb{R}^{\name{heads} \times \name{layer} \times \name{key}} \\
  ...
  M_{\nidx{seq}{i}, \nidx{seq'}{j}} &= \begin{cases}
    ...
    -\infty & \text{otherwise}
  \end{cases} \\
  Y &= W^{l,O} \ndot{heads,val} \text{Att}(Q, K, V, M) & W^{l,O} &\in \mathbb{R}^{\name{heads} \times \name{val} \times \name{layer}}
\end{align*}
