...
If $Q$, $K$, and $V$ have a \name{head} axis for multiple attention heads, then the above equations will compute multi-head attention without modification.
...
U &\in \reals^{\name{channels} \times \nset{seq}{n-|\name{kernel}|+1} \times \name{kernel}} \\
U_{\nidx{seq}{i}, \nidx{kernel}{j}} & = X_{\nidx{seq}{i+j - 1}} \\
\text{conv1d}(X; W, b) &= W \ndot{channels, kernel} U + b \in \mathbb{R}^{\nset{seq}{n-|\name{kernel}|+1}}.
\end{align*} 
...
\nset{width}{w-|\name{kw}|+1} \times \name{kh} \times \name{kw}}  \\
U_{\nidx{height}{i}, \nidx{width}{j}, \nidx{kh}{ki}, \nidx{kw}{kj}} &= X_{\nidx{height}{i+ki-1}, \nidx{width}{j+kj-1}} \\
\text{conv2d}(X; W, b) &=  W \ndot{channels, kh, kw} U + b \in \reals^{\nset{height}{h-|\name{kh}|+1} \times \nset{width}{w-|\name{kw}|+1}}.
\end{align*}
