...
\begin{align*}
  k &= u \ndot{ax1} v && \text{inner product} \\
  A &= u \odot x && \text{outer product} \\
  y &= A \ndot{ax2} x && \text{matrix-vector multiplication} \\
  v &= u \ndot{ax1} A && \text{vector-matrix multiplication} \\
  C &= A \ndot{ax2} B && \text{matrix-matrix multiplication}
\end{align*}
...
X^2 &= \text{MaxPool}^2(T^2)_{\nmov{(height,width,channels)}{layer}} \\
