...
\newcommand{\nndot}[2]{\mathbin{\mathop{\odot}\limits_{\name{#1}|\name{#2}}}}
...
In a \emph{named tensor}, we give each axis a name. For example, if the above tensor represents an image, we can name the axes as follows:
\begin{align*}
  A &\in \reals^{\nset{height}{3} \times \nset{width}{3}} = \reals^{\nset{width}{3} \times \nset{height}{3}} \\
  A &= \nmatrix{height}{width}{
    3 & 1 & 4 \\
...
\end{align*}
We wrote $A$ two equivalent ways to show that the order of axes does not matter.
...
What are good choices for axis names? We recommend meaningful \emph{words} instead of single letters, and we recommend words that describe a \emph{whole} rather than its parts. For example, if we extended the example $A$ above to have red, green, and blue channels, we'd name the axis \name{channels}, and if we wanted to represent a minibatch of images, we'd name the axis \name{batch}. Please see \S\ref{sec:examples} for more examples.
...
Any function from scalars to scalars can be applied elementwise to a named tensor. For example:
\begin{equation*}
\exp A = \nmatrix{height}{width}{
  \exp 3 & \exp 1 & \exp 4 \\
  \exp 1 & \exp 5 & \exp 9 \\
...
Similarly, any function or operator from two scalars to a scalar can be applied elementwise to two named tensors with the same shape.
\begin{equation*}
\frac1{1+\exp(-A)} = \nmatrix{height}{width}{
...
\end{equation*}
We write $\odot$ for elementwise (Hadamard) product.

But if two tensors have different shapes, they are \emph{broadcast} against each other (similarly to NumPy and its derivatives). Suppose
\begin{align*}
...
Again, you can write multiple names to contract multiple axes at once. An operator $\odot$ with no axis name under it contracts zero axes and is equivalent to elementwise multiplication (which is why we use the same symbol for both).

\subsubsection{Vector-valued operations}

A very common example of a function from vectors to vectors is the softmax:
\begin{align*}
  \nfun{height}{softmax} A &= \frac{\exp A}{\nsum{height} \exp A} \approx \nmatrix{height}{width}{
    0.67 & 0.00 & 0.01 \\
    0.09 & 0.27 & 0.97 \\
    0.24 & 0.73 & 0.02
  }.
%  \nfun{width}{softmax} A &= \frac{\exp A}{\nsum{width} \exp A} \approx \nmatrix{height}{width}{
%    0.26 & 0.04 & 0.70 \\
%    0.00 & 0.02 & 0.98 \\
%    0.01 & 0.72 & 0.27
%  }.
...
All of the tensor operations described in \S\ref{sec:operations} can be defined in this way.
\paragraph{Reductions}
\begin{align*}
  \nsum{ax} A &= \sum_{i=1}^n A_{\nidx{ax}{i}} \\
...
\end{align*}
Note that, like vector dot-product, this operator is commutative, but not associative.
Specifically, if
\begin{align*}
A &\in \mathbb{R}^{\nset{foo}{m}} \\
B &\in \mathbb{R}^{\nset{foo}{m} \times\nset{bar}{n}} \\
...
\begin{align*}
  ...
  \nmov{ax}{ax'}{\cdot} \colon F^{\nset{ax}{n}} &\rightarrow F^{\nset{ax'}{n}} \\
  \left[\nmov{ax}{ax'}{A}\right]_{\nidx{ax'}{i}} &= A_{\nidx{ax}{i}}
\end{align*}

\paragraph{Matrices}

We can also define functions on order-two tensors. These have two axis names written under them, and their order in general matters (though not for these two examples).
\begin{align*}
