...
\paragraph{Reductions} ($F^{\ax[n]} \rightarrow F$)
\begin{align*}
...
\paragraph{Contraction} ($F^{\ax[n]} \times F^{\ax[n]} \rightarrow F$)
\begin{align*}
  A \ndot{\ax} B &= \sum_{i=1}^n A_{\ax(i)} B_{\ax(i)}
\end{align*}

\paragraph{Vectors to vectors} ($F^{\ax[n]} \rightarrow F^{\ax[n]}$)
\begin{align*}
...
\paragraph{Renaming} ($F^{\ax[n]} \rightarrow F^{\ax'[n]}$)
\begin{align*}
